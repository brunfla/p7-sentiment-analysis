...
def main():
    ...
    unique_classes = df['id'].unique()
    logger.info(f"Classes uniques dans la colonne 'id' : {unique_classes}")
    logger.info(f"Nombre de classes uniques : {len(unique_classes)}")

    # Transformer les classes 4 en 1 pour harmoniser les labels
    logger.info("Transformation des classes : 4 -> 1")
    df['id'] = df['id'].apply(lambda x: 1 if x == 4 else x)
    logger.info(f"Classes uniques après transformation : {df['id'].unique()}")
